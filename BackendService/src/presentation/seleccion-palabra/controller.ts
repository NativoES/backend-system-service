...
export class SeleccionPalabrasController {
  constructor(private readonly service: SeleccionPalabrasService) {}
  ...
  create = (req: Request, res: Response) => {
    const [error, dto] = RegisterSeleccionPalabrasDto.create(req.body);
  ...
  update = (req: Request, res: Response) => {
    const { id } = req.params;
    const [error, dto] = UpdateSeleccionPalabrasDto.create(req.body);
    if (error) return res.status(400).json({ error });
