...
export class EnlacesExternosController {
  constructor(private readonly service: EnlaceExternoService) {}
  ...
  create = (req: Request, res: Response) => {
    const [error, dto] = RegisterEnlaceExternoDto.create(req.body);
  ...
  update = (req: Request, res: Response) => {
    const { id } = req.params;
    const [error, dto] = UpdateEnlaceExternoDto.create(req.body);
    if (error) return res.status(400).json({ error });
