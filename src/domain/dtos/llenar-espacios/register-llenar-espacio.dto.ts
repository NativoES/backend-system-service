export class RegisterLlenarEspaciosDto {
  private constructor(
  ...
  static create(object: { [key: string]: any }): [string?, RegisterLlenarEspaciosDto?] {
    ...
    if (!Array.isArray(palabras) || palabras.some(p => typeof p !== 'string')) {
      return ['El campo "palabras" es requerido y debe ser un array de strings'];
    }
    ...
    return [undefined, new RegisterLlenarEspaciosDto(titulo, textoOriginal, palabras, claseId, template, descripcion)];
  }
}
