export class UpdateVideoDto {
  ...
  static create(object: { [key: string]: any }): [string?, UpdateVideoDto?] {
    const allowedFields = ['titulo', 'videoUrl', 'claseId', 'template', 'descripcion'];
    ...
    const { titulo, videoUrl, claseId, template, descripcion } = object;

    if (titulo !== undefined && typeof titulo !== 'string') return ['"titulo" debe ser string'];
    if (videoUrl !== undefined && typeof videoUrl !== 'string') return ['"videoUrl" debe ser string'];
    ...
    return [undefined, new UpdateVideoDto(titulo, videoUrl, claseId, template, descripcion)];
  }
}
